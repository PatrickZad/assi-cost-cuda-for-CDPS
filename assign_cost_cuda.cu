...
int **copy_mat_to_dev(pybind11::array_t<int> mat, int *gpu_fl_ptr, int *mat_h_ptr, int *mat_w_ptr)
{
  ...
  *mat_w_ptr = w;
  ...
  int *host_data;
  //malloc
  HANDLE_ERROR(cudaMalloc((void **)(&gpu_mat_ptr), sizeof(int *) * h));
  HANDLE_ERROR(cudaMalloc((void **)(&gpu_fl_ptr), h * w * sizeof(int)));
...
pybind11::array_t<float> assign_cost_mat(pybind11::array_t<int> data_left, pybind11::array_t<int> batch_built, int p, int k)
{
  ...
  float *gpu_res_data;
  float *res_data;
  gpu_dl_ptr = copy_mat_to_dev(data_left, gpu_dl_fl_ptr, &dl_h, &dl_w);
  gpu_bb_ptr = copy_mat_to_dev(batch_built, gpu_bb_fl_ptr, &bb_h, &bb_w);
